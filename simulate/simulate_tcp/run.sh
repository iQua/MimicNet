...
python ../common/generate_config.py ${SEED} out/${UNIQUE_NAME}.cluster \
    out/${UNIQUE_NAME}.route results/${UNIQUE_NAME}_tcp --numClusters ${CLUSTERS} \
...
mkdir -p results/${UNIQUE_NAME}_tcp
if [[ "${VARIANT}" == "RecordTraining" || "${VARIANT}" == "RecordAll" ]]; then
    mkdir -p results/${UNIQUE_NAME}_tcp/hosts${CLUSTERS}
    mkdir -p results/${UNIQUE_NAME}_tcp/edges${CLUSTERS}
fi
if [[ "${VARIANT}" == "RecordEval" || "${VARIANT}" == "RecordAll" ]]; then
    mkdir -p results/${UNIQUE_NAME}_tcp/eval${CLUSTERS}
fi
...
# Run Simulation
echo -e "\e[34mRunning simulate_tcp\e[0m"
./simulate_tcp out/${UNIQUE_NAME}.ini -u Cmdenv -c ${VARIANT} ${NEDPATH}
echo -e "${ORIG_ARGS}" > results/${UNIQUE_NAME}_tcp/params.txt
echo -e "${SEED}" >> results/${UNIQUE_NAME}_tcp/params.txt
echo -e "${DEGREE}" >> results/${UNIQUE_NAME}_tcp/params.txt
