...
class SIM_API cSimulation : public cNamedObject, noncopyable
{
  ...
  private:
    ...
    std::map<const char*, cState*,cmp_str> statemap; //the map stores states, identified by the CLASSNAME of a module. (used by parallel initialization)
  ...
  public:
    ...
    /**
     * Parallel Initalization: Returns true if a module has registered for parinit via Register_ParallelInitialization_Module macro
     */
    bool moduleHasRegisterdForParallelInitialization(int moduleId);

    /**
     * Parallel Initalization: Returns weather a module has to wait for initialization to get the
     * current State from another LP which has registered via
     * Register_ParallelInitialization_Module macro.
     */
     bool hasToWaitForParallelInitialization(int moduleId, int stage);

    /**
     * Parallel Initalization: The functions look up the next / previous module of the same type as the module which is given as parameter
     * This is done by iterating through the module vector
     */
    int getParallelIntializationNextModuleId(int moduleId);
    int getParallelIntializationPrevModuleId(int moduleId);

    /**
     * Parallel Initalization: Returns only true if the distributed / parallel initialization has finished with the given stages  
     * and the module identified by the module id is the last module of this type in the module vector
     */
     bool isLastModule(int moduleid);

    /**
     *  Parallel Initalization: Convinience functions which are used by the modules to get the next / previous state.
     */
     cState* getPreState(int moduleid, int stage);
     void setPostState(int moduleid, cState *state, int stage);
