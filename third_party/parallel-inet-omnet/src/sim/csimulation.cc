...
bool cSimulation::moduleHasRegisterdForParallelInitialization(int moduleId)
{
    cModule *currentModule = getModule(moduleId);

    cRegistrationList *table = parInitModules.getInstance();

    return table->lookup(currentModule->getClassName()) != NULL;
}

bool cSimulation::hasToWaitForParallelInitialization(int moduleId, int stage)
{

    cModule *currentModule = getModule(moduleId);
    cRegistrationList *table = parInitModules.getInstance();
    if(table->lookup(currentModule->getClassName()) == NULL) //the module has not registered for parallel initialization, we can continue
        return false;

    //we need to wait if the state is greater then 0
     if(stage > 0)
         return true;

    //check if currentModule is the first module of this type in the module vector
    for(int i = 1; i <= last_id; ++i)  {
        if(vect[i] != NULL)
        {
            if(vect[i]->getId() == currentModule->getId()) //case 1: the currentModule is the first in to be initialized
            {
                return false;
            }
            else
            {
                //in the case it is a placeholder get the origin name of the class the cPlaceholder is representing
                if(vect[i]->isPlaceholder() && !strcmp(check_and_cast<cPlaceholderModule *>(vect[i])->getOriginClassName(),currentModule->getClassName()))
                {
                    return true;
                }
                if(!vect[i]->isPlaceholder() && vect[i]->getClassName() == currentModule->getClassName())
                {
                    return true;
                }
            }
...
int cSimulation::getParallelIntializationPrevModuleId(int moduleId)
{
    cModule *currentModule = getModule(moduleId);

    cRegistrationList *table = parInitModules.getInstance();
    if(table->lookup(currentModule->getClassName()) == NULL) //the module has not registered for parallel initialization
            throw cRuntimeError("%s has not registered for parallel Initialization. This function should never be called.");


    //find out the index of currentModule
    int currentModuleIndex = -1;
    for(int i = 1; i <= last_id; ++i)
    {
        if(vect[i] != NULL && vect[i] == currentModule)
        {
            currentModuleIndex = i;
            break;
        }
    }
    assert(currentModuleIndex != -1);

    //iterate from found index to the beginning (and return if we find the previous module)
    for(int i = currentModuleIndex - 1; i > 0; --i)
    {
        if(vect[i] != NULL && strcmp(vect[i]->getOriginClassName(),currentModule->getOriginClassName()) == 0)
        {
            return  vect[i]->getId();
        }
    }

    //iterate from the end to the found index (and return if we find the next module)
    for(int i = last_id; i >= currentModuleIndex + 1; i--)
    {
        if(vect[i] != NULL && strcmp(vect[i]->getOriginClassName(),currentModule->getOriginClassName()) == 0)
        {
            return  vect[i]->getId();
    ...
    throw cRuntimeError("The previous next id could not be found.");

    return -1;
}

int cSimulation::getParallelIntializationNextModuleId(int moduleId)
{
    ...
    //iterate from found index to the end (and return if we find the next module)
    for(int i = currentModuleIndex + 1; i <= last_id; i++)
    {
        if(vect[i] != NULL && strcmp(vect[i]->getOriginClassName(),currentModule->getOriginClassName()) == 0)
        {
            return vect[i]->getId();
    ...
    //iterate from the beginning to the found index (and return if we find the next module)
    for(int i = 0; i <= currentModuleIndex - 1; i++)
    {
        if(vect[i] != NULL && strcmp(vect[i]->getOriginClassName(),currentModule->getOriginClassName()) == 0)
        {
            return vect[i]->getId();
    ...
    throw cRuntimeError("The next id could not be found.");
...
cState* cSimulation::getPreState(int moduleid, int stage)
{

    if(!hasToWaitForParallelInitialization(moduleid,stage)) //only for the first module of each registered type
    {


       cModule *mod = getModule(moduleid);

       return new cState(mod->getName(),moduleid);


    }
    else // wait until the state is received
    {
        EnvirBase *envirb = dynamic_cast<EnvirBase *>(evPtr);
        assert(envirb);

      if(ev.getConfig()->getAsBool(CFGID_PARALLEL_SIMULATION)) //parsim
      {
          //get the procid from the other module so that we can send to it
          int previousModuleId = getParallelIntializationPrevModuleId(moduleid);



          cModule *mod = getModule(moduleid);
          cModule *remoteMod = getModule(previousModuleId);

          if(remoteMod->isPlaceholder())
          {
              cPlaceholderModule *placeholderMod = dynamic_cast<cPlaceholderModule *>(remoteMod);


              //check if we received the desired state before
              std::map<const char*,cState*>::iterator it = statemap.find(mod->getClassName());
              if(it != statemap.end())
              {


                  cState *nextState = it->second;
                  statemap.erase(it);
                  return nextState;

              }
              else  //receive state from other LP
              {

                    cState *receivedState = envirb->parsimReceiveStateBlocking(placeholderMod->getRemoteProcId());

                    cModule *ownerMod = getModule(receivedState->getModuleId());

                    //check if the received State has the correct type
                    if(strcmp(ownerMod->getOriginClassName(),mod->getOriginClassName())==0)
                    {
                    ...
                    else
                    {
                        ...
                        //store the state temporarily in map and wait for the correct state
                        statemap.insert(std::pair<const char *, cState*>(ownerMod->getOriginClassName(),receivedState));
                        return getPreState(moduleid,stage);
                    }
              }
      ...
      {
          cModule *mod = getModule(moduleid);
           std::map<const char*,cState*>::const_iterator it = statemap.find(mod->getClassName());
          assert(it != statemap.end());
...
void cSimulation::setPostState(int moduleid, cState *state, int stage)
{
    if(ev.getConfig()->getAsBool(CFGID_PARALLEL_SIMULATION)) //parsim
    {
        //we only want to send until we reached the last stage and the last module of the stage
        int nextModuleId = getParallelIntializationNextModuleId(moduleid);

        cModule *module = getModule(moduleid);
        int maxInitStages = module->numInitStages();

        cModule *remoteMod = getModule(nextModuleId);
        if(!(stage == (maxInitStages-1) && isLastModule(moduleid))) //we finished the initialization
        {

            if(remoteMod->isPlaceholder()) // module located on another LP
            {
                cPlaceholderModule *placeholderMod = dynamic_cast<cPlaceholderModule *>(remoteMod);


                EnvirBase *envirb = dynamic_cast<EnvirBase *>(evPtr);
                envirb->parsimSendState(state,placeholderMod->getRemoteProcId());
            }
            else //module is located on our LP -> save it in statemap
            {
                statemap.insert(std::pair<const char *, cState*>(remoteMod->getClassName(),state));
            }

        }
        else //we finished parallel init
        {
    ...
    else
    {
        ...
        statemap.insert(std::pair<const char*, cState*>(mod->getClassName(),state));
    }
}

bool cSimulation::isLastModule(int moduleid)
{
    ...
    for(int i = last_id; i >= 1; --i)  {
       if(vect[i] != NULL)
       {
           ...
           if(vect[i]->getId() == currentModule->getId())
               return true;

           if(!strcmp(vect[i]->getOriginClassName(),currentModule->getOriginClassName()))
               return false;
